...
internal fun _QueryType(value: Int) = when(value) {
	...
	1000023000 -> QueryType.RESULT_STATUS_ONLY
	...
	1000299000 -> QueryType.VIDEO_ENCODE_BITSTREAM_BUFFER_RANGE
	else -> throw RuntimeException("Invalid enum: $value")
}
...
internal fun _ImageLayout(value: Int) = when(value) {
	...
	1000024000 -> ImageLayout.VIDEO_DECODE_DST
	1000024001 -> ImageLayout.VIDEO_DECODE_SRC
	1000024002 -> ImageLayout.VIDEO_DECODE_DPB
	1000111000 -> ImageLayout.SHARED_PRESENT
	1000218000 -> ImageLayout.FRAGMENT_DENSITY_MAP_OPTIMAL
	1000164003 -> ImageLayout.FRAGMENT_SHADING_RATE_ATTACHMENT_OPTIMAL
	1000299000 -> ImageLayout.VIDEO_ENCODE_DST
	1000299001 -> ImageLayout.VIDEO_ENCODE_SRC
	1000299002 -> ImageLayout.VIDEO_ENCODE_DPB
...
internal fun _DynamicState(value: Int) = when(value) {
	...
	1000352000 -> DynamicState.VERTEX_INPUT
	1000377000 -> DynamicState.PATCH_CONTROL_POINTS
	1000377001 -> DynamicState.RASTERIZER_DISCARD_ENABLE
	1000377002 -> DynamicState.DEPTH_BIAS_ENABLE
	1000377003 -> DynamicState.LOGIC_OP
	1000377004 -> DynamicState.PRIMITIVE_RESTART_ENABLE
	1000381000 -> DynamicState.COLOR_WRITE_ENABLE
	else -> throw RuntimeException("Invalid enum: $value")
}
...
internal fun _AttachmentLoadOp(value: Int) = when(value) {
	...
	1000400000 -> AttachmentLoadOp.NONE
	else -> throw RuntimeException("Invalid enum: $value")
}
...
internal fun _PipelineBindPoint(value: Int) = when(value) {
	...
	1000369003 -> PipelineBindPoint.SUBPASS_SHADING
	else -> throw RuntimeException("Invalid enum: $value")
}
...
internal fun _QueryResultStatus(value: Int) = when(value) {
	-1 -> QueryResultStatus.ERROR
	0 -> QueryResultStatus.NOT_READY
	1 -> QueryResultStatus.COMPLETE
	else -> throw RuntimeException("Invalid enum: $value")
}
...
internal fun _ValidationFeatureDisable(value: Int) = when(value) {
	...
	7 -> ValidationFeatureDisable.SHADER_VALIDATION_CACHE
	else -> throw RuntimeException("Invalid enum: $value")
}
...
internal fun _ProvokingVertexMode(value: Int) = when(value) {
	0 -> ProvokingVertexMode.FIRST_VERTEX
	1 -> ProvokingVertexMode.LAST_VERTEX
	else -> throw RuntimeException("Invalid enum: $value")
}
...
internal fun _AccelerationStructureMotionInstanceType(value: Int) = when(value) {
	0 -> AccelerationStructureMotionInstanceType.STATIC
	1 -> AccelerationStructureMotionInstanceType.MATRIX_MOTION
	2 -> AccelerationStructureMotionInstanceType.SRT_MOTION
	else -> throw RuntimeException("Invalid enum: $value")
}