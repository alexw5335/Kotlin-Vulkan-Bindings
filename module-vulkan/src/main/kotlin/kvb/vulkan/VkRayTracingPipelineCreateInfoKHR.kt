 ...
 */
@JvmInline
value class VkRayTracingPipelineCreateInfoKHR(override val address: Long) : Addressable {
	...
	var groups: VkRayTracingShaderGroupCreateInfoKHR.Buffer
		get()      = VkRayTracingShaderGroupCreateInfoKHR.Buffer(Unsafe.getLong(address + 40), groupCount)
	...
	class Buffer(override val address: Long, override val capacity: Int) : DirectBuffer {
		...
		operator fun get(index: Int) = VkRayTracingPipelineCreateInfoKHR(offset(index))
		
		operator fun set(index: Int, value: VkRayTracingPipelineCreateInfoKHR) = Unsafe.copy(value.address, offset(index), 104)
		
		inline fun forEach(block: (VkRayTracingPipelineCreateInfoKHR) -> Unit) = repeat(capacity) { block(get(it)) }
		
		inline fun<R> map(block: (VkRayTracingPipelineCreateInfoKHR) -> R) = List(capacity) { block(get(it)) }
		
		inline fun<R> mapIndexed(block: (Int, VkRayTracingPipelineCreateInfoKHR) -> R) = List(capacity) { block(it, get(it)) }
