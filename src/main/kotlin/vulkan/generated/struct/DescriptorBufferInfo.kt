...
value class DescriptorBufferInfo(override val address: Long) : Addressable {
	...
	class Buffer(override val address: Long, override val capacity: Int) : DirectBuffer {
		...
		operator fun set(index: Int, value: DescriptorBufferInfo) = Unsafe.copy(value.address, offset(index), SIZE)
		...
		override fun toString() = "DescriptorBufferInfo[$capacity]"
	...
	companion object {
		...
		val NULL = DescriptorBufferInfo(0)
		
		val EMPTY = Buffer(0, 0)
		
		fun malloc(allocator: Allocator) = DescriptorBufferInfo(allocator.malloc(SIZE, ALIGNMENT))
		
		fun calloc(allocator: Allocator) = DescriptorBufferInfo(allocator.calloc(SIZE, ALIGNMENT))
