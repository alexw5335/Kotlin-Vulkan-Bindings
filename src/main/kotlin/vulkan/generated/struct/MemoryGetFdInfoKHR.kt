 ...
 */
@JvmInline
value class MemoryGetFdInfoKHR(override val address: Long) : Addressable {
	...
	companion object {
		...
		val NULL = MemoryGetFdInfoKHR(0)
		
		fun malloc(allocator: Allocator) = MemoryGetFdInfoKHR(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.MEMORY_GET_FD_INFO
		}
		
		fun calloc(allocator: Allocator) = MemoryGetFdInfoKHR(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.MEMORY_GET_FD_INFO
		}
