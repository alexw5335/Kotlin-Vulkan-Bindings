...
value class Win32KeyedMutexAcquireReleaseInfoKHR(override val address: Long) : Addressable {
	...
	companion object {
		...
		val NULL = Win32KeyedMutexAcquireReleaseInfoKHR(0)
		
		fun malloc(allocator: Allocator) = Win32KeyedMutexAcquireReleaseInfoKHR(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.WIN32_KEYED_MUTEX_ACQUIRE_RELEASE_INFO_KHR
		}
		
		fun calloc(allocator: Allocator) = Win32KeyedMutexAcquireReleaseInfoKHR(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.WIN32_KEYED_MUTEX_ACQUIRE_RELEASE_INFO_KHR
		}
