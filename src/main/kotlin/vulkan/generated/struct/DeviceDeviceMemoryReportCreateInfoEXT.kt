 ...
 */
@JvmInline
value class DeviceDeviceMemoryReportCreateInfoEXT(override val address: Long) : Addressable {
	...
	companion object {
		...
		val NULL = DeviceDeviceMemoryReportCreateInfoEXT(0)
		
		fun malloc(allocator: Allocator) = DeviceDeviceMemoryReportCreateInfoEXT(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.DEVICE_DEVICE_MEMORY_REPORT_CREATE_INFO
		}
		
		fun calloc(allocator: Allocator) = DeviceDeviceMemoryReportCreateInfoEXT(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.DEVICE_DEVICE_MEMORY_REPORT_CREATE_INFO
		}
