...
value class SparseImageMemoryRequirements2(override val address: Long) : Addressable {
	...
	var memoryRequirements: SparseImageMemoryRequirements
		get()      = SparseImageMemoryRequirements(address + LAYOUT.offsets[2])
	...
	class Buffer(override val address: Long, override val capacity: Int) : DirectBuffer {
		...
		operator fun set(index: Int, value: SparseImageMemoryRequirements2) = Unsafe.copy(value.address, offset(index), SIZE)
		...
		override fun toString() = "SparseImageMemoryRequirements2[$capacity]"
	...
	class Persistent(
		...
		val memoryRequirements: SparseImageMemoryRequirements.Persistent,
	) {
		override fun toString() = "SparseImageMemoryRequirements2(" + 
			"sType=$sType, " +
	...
	companion object {
		...
		val LAYOUT = LayoutBuilder.buildStruct {
			...
			member(SparseImageMemoryRequirements.LAYOUT)
		}
		...
		val NULL = SparseImageMemoryRequirements2(0)
		
		val EMPTY = Buffer(0, 0)
		
		fun malloc(allocator: Allocator) = SparseImageMemoryRequirements2(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.SPARSE_IMAGE_MEMORY_REQUIREMENTS_2
		}
		
		fun calloc(allocator: Allocator) = SparseImageMemoryRequirements2(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.SPARSE_IMAGE_MEMORY_REQUIREMENTS_2
		}
		
		fun malloc(allocator: Allocator, capacity: Int) = Buffer(allocator.malloc(SIZE*capacity, ALIGNMENT), capacity).apply {
			forEach { it.sType = StructureType.SPARSE_IMAGE_MEMORY_REQUIREMENTS_2 }
		}
