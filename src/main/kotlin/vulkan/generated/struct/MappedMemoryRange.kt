...
value class MappedMemoryRange(override val address: Long) : Addressable {
	...
	class Buffer(override val address: Long, override val capacity: Int) : DirectBuffer {
		...
		operator fun set(index: Int, value: MappedMemoryRange) = Unsafe.copy(value.address, offset(index), SIZE)
		...
		override fun toString() = "MappedMemoryRange[$capacity]"
	...
	companion object {
		...
		val NULL = MappedMemoryRange(0)
		
		val EMPTY = Buffer(0, 0)
		
		fun malloc(allocator: Allocator) = MappedMemoryRange(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.MAPPED_MEMORY_RANGE
		}
		
		fun calloc(allocator: Allocator) = MappedMemoryRange(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.MAPPED_MEMORY_RANGE
		}
		
		fun malloc(allocator: Allocator, capacity: Int) = Buffer(allocator.malloc(SIZE*capacity, ALIGNMENT), capacity).apply {
			forEach { it.sType = StructureType.MAPPED_MEMORY_RANGE }
		}
