 ...
 */
@JvmInline
value class ResolveImageInfo2KHR(override val address: Long) : Addressable {
	...
	var regions: ImageResolve2KHR.Buffer
		get()      = ImageResolve2KHR.Buffer(Unsafe.getNativePointer(address + LAYOUT.offsets[7]), regionCount)
	...
	companion object {
		...
		val NULL = ResolveImageInfo2KHR(0)
		
		fun malloc(allocator: Allocator) = ResolveImageInfo2KHR(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.RESOLVE_IMAGE_INFO_2
		}
		
		fun calloc(allocator: Allocator) = ResolveImageInfo2KHR(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.RESOLVE_IMAGE_INFO_2
		}
