 ...
 */
@JvmInline
value class ImageCopy2KHR(override val address: Long) : Addressable {
	...
	class Buffer(override val address: Long, override val capacity: Int) : DirectBuffer {
		...
		operator fun get(index: Int) = ImageCopy2KHR(offset(index))
		
		operator fun set(index: Int, value: ImageCopy2KHR) = Unsafe.copy(value.address, offset(index), SIZE)
		
		inline fun forEach(block: (ImageCopy2KHR) -> Unit) = repeat(capacity) { block(get(it)) }
		
		inline fun<R> map(block: (ImageCopy2KHR) -> R) = List(capacity) { block(get(it)) }
		
		override fun toString() = "ImageCopy2KHR[$capacity]"
	...
	companion object {
		...
		val NULL = ImageCopy2KHR(0)
		
		val EMPTY = Buffer(0, 0)
		
		fun malloc(allocator: Allocator) = ImageCopy2KHR(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.IMAGE_COPY_2
		}
		
		fun calloc(allocator: Allocator) = ImageCopy2KHR(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.IMAGE_COPY_2
		}
		
		fun malloc(allocator: Allocator, capacity: Int) = Buffer(allocator.malloc(SIZE*capacity, ALIGNMENT), capacity).apply {
			forEach { it.sType = StructureType.IMAGE_COPY_2 }
		}
