 ...
 */
@JvmInline
value class FenceGetFdInfoKHR(override val address: Long) : Addressable {
	...
	companion object {
		...
		val NULL = FenceGetFdInfoKHR(0)
		
		fun malloc(allocator: Allocator) = FenceGetFdInfoKHR(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.FENCE_GET_FD_INFO
		}
		
		fun calloc(allocator: Allocator) = FenceGetFdInfoKHR(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.FENCE_GET_FD_INFO
		}
