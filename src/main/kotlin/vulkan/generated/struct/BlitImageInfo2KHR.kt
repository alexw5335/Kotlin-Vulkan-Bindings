 ...
 */
@JvmInline
value class BlitImageInfo2KHR(override val address: Long) : Addressable {
	...
	var regions: ImageBlit2KHR.Buffer
		get()      = ImageBlit2KHR.Buffer(Unsafe.getNativePointer(address + LAYOUT.offsets[7]), regionCount)
	...
	companion object {
		...
		val NULL = BlitImageInfo2KHR(0)
		
		fun malloc(allocator: Allocator) = BlitImageInfo2KHR(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.BLIT_IMAGE_INFO_2
		}
		
		fun calloc(allocator: Allocator) = BlitImageInfo2KHR(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.BLIT_IMAGE_INFO_2
		}
