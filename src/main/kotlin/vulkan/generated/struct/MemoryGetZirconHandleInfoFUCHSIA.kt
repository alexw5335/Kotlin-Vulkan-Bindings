 ...
 */
@JvmInline
value class MemoryGetZirconHandleInfoFUCHSIA(override val address: Long) : Addressable {
	...
	companion object {
		...
		val NULL = MemoryGetZirconHandleInfoFUCHSIA(0)
		
		fun malloc(allocator: Allocator) = MemoryGetZirconHandleInfoFUCHSIA(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.MEMORY_GET_ZIRCON_HANDLE_INFO
		}
		
		fun calloc(allocator: Allocator) = MemoryGetZirconHandleInfoFUCHSIA(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.MEMORY_GET_ZIRCON_HANDLE_INFO
		}
