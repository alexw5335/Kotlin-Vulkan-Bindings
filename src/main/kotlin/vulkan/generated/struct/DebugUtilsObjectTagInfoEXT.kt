 ...
 */
@JvmInline
value class DebugUtilsObjectTagInfoEXT(override val address: Long) : Addressable {
	...
	companion object {
		...
		val NULL = DebugUtilsObjectTagInfoEXT(0)
		
		fun malloc(allocator: Allocator) = DebugUtilsObjectTagInfoEXT(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.DEBUG_UTILS_OBJECT_TAG_INFO
		}
		
		fun calloc(allocator: Allocator) = DebugUtilsObjectTagInfoEXT(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.DEBUG_UTILS_OBJECT_TAG_INFO
		}
