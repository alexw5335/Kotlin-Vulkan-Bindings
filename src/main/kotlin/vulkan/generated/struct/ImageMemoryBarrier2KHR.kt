 ...
 */
@JvmInline
value class ImageMemoryBarrier2KHR(override val address: Long) : Addressable {
	...
	var oldLayout: ImageLayout
		get()      = ImageLayout(Unsafe.getInt(address + LAYOUT.offsets[6]))
	...
	var image: ImageH
		get()      = ImageH(Unsafe.getNativePointer(address + LAYOUT.offsets[10]))
		set(value) { Unsafe.setNativePointer(address + LAYOUT.offsets[10], value.address) }
	
	var subresourceRange: ImageSubresourceRange
		get()      = ImageSubresourceRange(address + LAYOUT.offsets[11])
		set(value) { Unsafe.copy(value.address, address + LAYOUT.offsets[11], value.structSize) }
	...
	class Buffer(override val address: Long, override val capacity: Int) : DirectBuffer {
		...
		operator fun get(index: Int) = ImageMemoryBarrier2KHR(offset(index))
		
		operator fun set(index: Int, value: ImageMemoryBarrier2KHR) = Unsafe.copy(value.address, offset(index), SIZE)
		
		inline fun forEach(block: (ImageMemoryBarrier2KHR) -> Unit) = repeat(capacity) { block(get(it)) }
		
		inline fun<R> map(block: (ImageMemoryBarrier2KHR) -> R) = List(capacity) { block(get(it)) }
		
		override fun toString() = "ImageMemoryBarrier2KHR[$capacity]"
	...
	companion object {
		...
		val NULL = ImageMemoryBarrier2KHR(0)
		
		val EMPTY = Buffer(0, 0)
		
		fun malloc(allocator: Allocator) = ImageMemoryBarrier2KHR(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.IMAGE_MEMORY_BARRIER_2
		}
		
		fun calloc(allocator: Allocator) = ImageMemoryBarrier2KHR(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.IMAGE_MEMORY_BARRIER_2
		}
		
		fun malloc(allocator: Allocator, capacity: Int) = Buffer(allocator.malloc(SIZE*capacity, ALIGNMENT), capacity).apply {
			forEach { it.sType = StructureType.IMAGE_MEMORY_BARRIER_2 }
		}
