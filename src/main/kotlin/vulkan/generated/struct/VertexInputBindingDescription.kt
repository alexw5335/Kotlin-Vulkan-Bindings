...
value class VertexInputBindingDescription(override val address: Long) : Addressable {
	...
	var inputRate: VertexInputRate
		get()      = VertexInputRate(Unsafe.getInt(address + LAYOUT.offsets[2]))
	...
	class Buffer(override val address: Long, override val capacity: Int) : DirectBuffer {
		...
		operator fun set(index: Int, value: VertexInputBindingDescription) = Unsafe.copy(value.address, offset(index), SIZE)
		...
		override fun toString() = "VertexInputBindingDescription[$capacity]"
	...
	companion object {
		...
		val NULL = VertexInputBindingDescription(0)
		
		val EMPTY = Buffer(0, 0)
		
		fun malloc(allocator: Allocator) = VertexInputBindingDescription(allocator.malloc(SIZE, ALIGNMENT))
		
		fun calloc(allocator: Allocator) = VertexInputBindingDescription(allocator.calloc(SIZE, ALIGNMENT))
