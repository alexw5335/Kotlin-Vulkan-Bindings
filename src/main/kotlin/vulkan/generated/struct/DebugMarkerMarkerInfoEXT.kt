 ...
 */
@JvmInline
value class DebugMarkerMarkerInfoEXT(override val address: Long) : Addressable {
	...
	companion object {
		...
		val NULL = DebugMarkerMarkerInfoEXT(0)
		
		fun malloc(allocator: Allocator) = DebugMarkerMarkerInfoEXT(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.DEBUG_MARKER_MARKER_INFO
		}
		
		fun calloc(allocator: Allocator) = DebugMarkerMarkerInfoEXT(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.DEBUG_MARKER_MARKER_INFO
		}
