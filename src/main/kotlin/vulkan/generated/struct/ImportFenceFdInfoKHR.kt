 ...
 */
@JvmInline
value class ImportFenceFdInfoKHR(override val address: Long) : Addressable {
	...
	companion object {
		...
		val NULL = ImportFenceFdInfoKHR(0)
		
		fun malloc(allocator: Allocator) = ImportFenceFdInfoKHR(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.IMPORT_FENCE_FD_INFO
		}
		
		fun calloc(allocator: Allocator) = ImportFenceFdInfoKHR(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.IMPORT_FENCE_FD_INFO
		}
