...
value class ImageResolve(override val address: Long) : Addressable {
	...
	class Buffer(override val address: Long, override val capacity: Int) : DirectBuffer {
		...
		operator fun set(index: Int, value: ImageResolve) = Unsafe.copy(value.address, offset(index), SIZE)
		...
		override fun toString() = "ImageResolve[$capacity]"
	...
	companion object {
		...
		val NULL = ImageResolve(0)
		
		val EMPTY = Buffer(0, 0)
		
		fun malloc(allocator: Allocator) = ImageResolve(allocator.malloc(SIZE, ALIGNMENT))
		
		fun calloc(allocator: Allocator) = ImageResolve(allocator.calloc(SIZE, ALIGNMENT))
