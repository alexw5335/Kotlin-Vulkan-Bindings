...
value class ExternalBufferProperties(override val address: Long) : Addressable {
	...
	class Persistent(
		...
		override fun toString() = "ExternalBufferProperties(" + 
			"sType=$sType, " +
	...
	companion object {
		...
		val NULL = ExternalBufferProperties(0)
		
		fun malloc(allocator: Allocator) = ExternalBufferProperties(allocator.malloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.EXTERNAL_BUFFER_PROPERTIES
		}
		
		fun calloc(allocator: Allocator) = ExternalBufferProperties(allocator.calloc(SIZE, ALIGNMENT)).also {
			it.sType = StructureType.EXTERNAL_BUFFER_PROPERTIES
		}
