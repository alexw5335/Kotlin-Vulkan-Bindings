...
object PrimitiveGenerator {
	...
	fun generate(directory: Path) {
		for(p in Primitive.values()) {
			writeDirectPrimitive(directory, p)
	...
	private fun writeDirectPrimitive(
		directory: Path,
		primitive: Primitive
	) = KWriter.write(directory, primitive.directName) {
		start {
			autogenComment()
			package_(GenConfig.primitivePackage)
	...
	private fun writeDirectPrimitiveBuffer(
		...
		start {
			autogenComment()
			package_(GenConfig.primitivePackage)
			imports(DirectBuffer::class, Unsafe::class)
			if(primitive == Primitive.BYTE) import(Charset::class)
		}
