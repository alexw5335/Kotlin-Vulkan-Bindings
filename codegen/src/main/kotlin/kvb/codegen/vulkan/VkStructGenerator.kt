...
object VkStructGenerator {
	...
	fun generate(directory: Path, structs: Iterable<VkTypeStruct>) {
		for(s in structs)
			printStruct(directory, s)
	}
	...
	private fun printStruct(directory: Path, struct: VkTypeStruct) = KWriter.write(directory, struct.genName) {
		start {
			autogenComment()
			package_(GenConfig.structPackage)
		...
		class_("value class $name(override val address: Long) : Addressable") {
			...
			if(struct.requiresBuffer) {
				class_("class Buffer(override val address: Long, override val capacity: Int) : DirectBuffer", style(1, 1)) {
					...
					declaration("fun copy(allocator: Allocator, newCapacity: Int = capacity * 2)")
				}
