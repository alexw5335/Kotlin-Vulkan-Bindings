...
class VulkanGenerator(
	...
	fun genEnumConstructors() = write("Enums") {
		start {
			autogenComment()
		...
		styled(style(1, 0)) {
			for(enum in registry.types) {
				if(enum !is EnumType || enum.isFlagBits || enum.isValue) continue
	...
	fun genBitmaskBlocks() = write("Bitmasks") {
		start {
			autogenComment()
			suppressFile("Unused", "FunctionName")
			vulkanPackage()
		...
		styled(style(1, 0)) {
			for(b in registry.types) {
				if(b !is BitmaskType) continue
	...
	private val Command.asCFunction get() = JniGen.createCFunction(
		...
		className    = registry.namesToProviders[name]!!.name.replace("_", "_1") + "Kt",
	...
	fun genProviders() {
		registry.providers.forEach(::genProvider)
	}
	...
	fun genStructAllocations() = write("Allocations") {
		start {
			autogenComment()
			suppressFile("Unused", "FunctionName")
			vulkanPackage()
		...
		for(struct in registry.types) {
			if(struct !is StructType) continue

			val type = struct.genName

			if(struct.sType != null) {
				declaration("inline fun Allocator.$type(block: ($type) -> Unit) = $type(calloc(${struct.size64})).apply(block).also { it.sType = ${struct.sType} }")

				if(struct.requiresBuffer)
					declaration("inline fun Allocator.$type(capacity: Int, block: ($type.Buffer) -> Unit) = $type.Buffer(calloc(capacity * ${struct.size64}), capacity).apply(block).apply { forEach { it.sType = ${struct.sType} } }")
			} else {
				declaration("inline fun Allocator.$type(block: ($type) -> Unit) = $type(calloc(${struct.size64})).apply(block)")

				if(struct.requiresBuffer)
					declaration("inline fun Allocator.$type(capacity: Int, block: ($type.Buffer) -> Unit) = $type.Buffer(calloc(capacity * ${struct.size64}), capacity).apply(block)")
			}
		}
	}
