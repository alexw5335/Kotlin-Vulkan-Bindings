...
value class RayTracingPipelineCreateInfoNV(override val address: Long) : Addressable {
	...
	var groups: RayTracingShaderGroupCreateInfoNV.Buffer
		get()      = RayTracingShaderGroupCreateInfoNV.Buffer(Unsafe.getLong(address + 40), groupCount)
	...
	class Buffer(override val address: Long, override val capacity: Int) : DirectBuffer {
		...
		operator fun set(index: Int, value: RayTracingPipelineCreateInfoNV) = Unsafe.copy(value.address, offset(index), 80)
		...
		inline fun<R> mapIndexed(block: (Int, RayTracingPipelineCreateInfoNV) -> R) = List(capacity) { block(it, get(it)) }
